...
IFX_INTERRUPT(cc60_pit_ch0_isr, 0, CCU6_0_CH0_ISR_PRIORITY)
{
	...
	leftspeed=-8000*gpt12_get(GPT12_T3)/544;    //0.01cm/0.01s ->
	rightspeed=8000*gpt12_get(GPT12_T2)/544;
    if (times % 2 == 0)
    {
        carControl();
        times = 0;
    }
	carCtrl.differ = pid_servo(differ);
...
IFX_INTERRUPT(eru_ch3_ch7_isr, 0, ERU_CH3_CH7_INT_PRIO)
{
	...
	if(GET_GPIO_FLAG(ERU_CH3_REQ6_P02_0))//通道3中断
	{
		...
		if (CAMERA_BIN_UART  == camera_type)	ov7725_uart_vsync();
		else if(CAMERA_GRAYSCALE==camera_type)  mt9v03x_vsync();
...
IFX_INTERRUPT(dma_ch5_isr, 0, ERU_DMA_INT_PRIO)
{
	...
	if (CAMERA_BIN_UART  == camera_type)	ov7725_uart_dma();
	else if (CAMERA_GRAYSCALE==camera_type) mt9v03x_dma();
}
